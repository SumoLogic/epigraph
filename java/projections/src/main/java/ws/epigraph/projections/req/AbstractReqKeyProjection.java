 ...
 */
public class AbstractReqKeyProjection {
  ...
  public AbstractReqKeyProjection(
      @NotNull Datum value,
  ...
  public boolean equals(Object o) {
    ...
    AbstractReqKeyProjection that = (AbstractReqKeyProjection) o;
  ...
  public static <RKP extends AbstractReqKeyProjection> List<RKP> merge(
...
//    if (keysToMerge.size() < 2) return keysToMerge;
    ...
    for (final Map.Entry<Datum, List<RKP>> entry : groupedKeysToMerge.entrySet()) {
      ...
      mergedKeys.add(
          factory.create(
              group,
              keyValue,
              ReqParams.merge(group.stream().map(AbstractReqKeyProjection::params)),
              Directives.merge(group.stream().map(AbstractReqKeyProjection::directives))
          )
      );
    }
  ...
  public interface MergedReqKeyFactory<RKP extends AbstractReqKeyProjection> {
    RKP create(
