...
// UNFINISHED ATTEMPT TO BUILD NORMALIZING PARSER
...
//              modelPropertiesPsi,
//              typesResolver,
//              getSiblingMetaModelProjections(tag.name(), siblings),
//              errors
//          ),
//          modelProjection,
//          typesResolver,
//          getSiblingModelProjections(tag.name(), siblings),
//          errors
//      );
//
//      tagProjections.put(
...
//    }
//
//    final List<OpOutputVarProjection> tails = parseTails(dataType, psi, typesResolver, siblings, errors);
//
//    try {
...
//
//  @Nullable
//  private static List<OpOutputVarProjection> parseTails(
//      final @NotNull DataType dataType,
//      final @NotNull IdlOpOutputVarProjection psi,
//      final @NotNull TypesResolver typesResolver,
//      final @NotNull List<OpOutputVarProjection> siblings,
...
//            buildTailProjection(dataType, tailTypeRef, psiTailProjection, typesResolver, siblings, errors);
...
//
//  @NotNull
//  private static List<OpOutputModelProjection<?, ?>> getSiblingMetaModelProjections(
//      @NotNull String tag,
//      @NotNull List<? extends OpOutputVarProjection> siblings) {
//
//    if (siblings.isEmpty()) return Collections.emptyList();
//
//    return siblings.stream()
//                   .map(mp -> mp.tagProjection(tag))
//                   .filter(Objects::nonNull)
//                   .map(tp -> tp.projection().metaProjection())
//                   .collect(Collectors.toList());
//
//  }
//
//  @NotNull
//  private static List<OpOutputModelProjection<?, ?>> getSiblingModelProjections(
//      @NotNull String tag,
//      @NotNull List<? extends OpOutputVarProjection> siblings) {
//
//    if (siblings.isEmpty()) return Collections.emptyList();
...
//                   .map(mp -> mp.tagProjection(tag))
//                   .filter(Objects::nonNull)
//                   .map(AbstractTagProjectionEntry::projection)
//                   .collect(Collectors.toList());
//
//  }
//
...
//      @NotNull List<IdlOpOutputModelProperty> modelProperties,
//      @NotNull TypesResolver resolver,
//      @NotNull List<? extends OpOutputModelProjection<?, ?>> siblings,
//      @NotNull List<PsiProcessingError> errors
//  ) throws PsiProcessingException {
//
...
//
//    List<OpOutputVarProjection> siblingTails =
//        siblings.isEmpty() ? Collections.emptyList() :
//        siblings.stream()
//                .map(vp -> vp.tailByType(tailType)).filter(Objects::nonNull).collect(Collectors.toList());
//
//    return parseVarProjection(
//        new DataType(tailType, varType.defaultTag),
//        psiTailProjection,
//        typesResolver,
//        siblingTails,
//        errors
