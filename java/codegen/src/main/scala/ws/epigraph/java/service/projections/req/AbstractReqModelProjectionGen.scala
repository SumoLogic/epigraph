 ...
 */
trait AbstractReqModelProjectionGen extends AbstractReqTypeProjectionGen {
  ...
  override protected type GenType <: AbstractReqModelProjectionGen
  ...
  protected def metaGenerator(metaOp: OpMetaProjectionType): AbstractReqModelProjectionGen =
  ...
  protected lazy val params: CodeChunk =
    AbstractReqProjectionGen.generateParams(op.params(), namespace.toString, "raw.params()")

  lazy val metaGeneratorOpt: Option[AbstractReqModelProjectionGen] = {
...
//    ).getOrElse(Map())
  ...
  lazy val normalizedTailGenerators: Map[OpProjectionType, AbstractReqModelProjectionGen] =
    Option(op.polymorphicTails()).map(
  ...
  private def genTail(tail: OpProjectionType, tailGenerator: AbstractReqModelProjectionGen): CodeChunk = CodeChunk.empty
//  {
...
//  }
  ...
  def genNormalizedTail(tail: OpProjectionType, tailGenerator: AbstractReqModelProjectionGen): CodeChunk = CodeChunk.empty
//  {
//    val tailCtype = JavaGenUtils.toCType(tail.`type`())
