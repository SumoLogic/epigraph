 ...
 */
public abstract class SyncEpigraphServlet extends EpigraphServlet {
  ...
  protected void handleRequest(HttpMethod method, HttpServletRequest req, HttpServletResponse resp) {
    try {
      ...
      ServletInvocationContext context = new ServletInvocationContext(method, req, resp);
  ...
  private final class ServletInvocationContext implements HttpInvocationContext {
  ...
  private class Server extends AbstractHttpServer<ServletInvocationContext> {
    ...
    protected void handleRequest(
        ...
        final @NotNull SyncEpigraphServlet.@NotNull ServletInvocationContext context) {
    ...
    protected void close(final @NotNull ServletInvocationContext context) throws IOException {
      context.response.getOutputStream().flush(); // this commits the response
    }
  }
