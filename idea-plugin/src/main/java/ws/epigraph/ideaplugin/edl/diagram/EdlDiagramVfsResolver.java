 ...
 */
public class EdlDiagramVfsResolver implements DiagramVfsResolver<PsiNamedElement> {
  ...
  public String getQualifiedName(PsiNamedElement element) {
    ...
    if (element instanceof EdlTypeDef) {
      ...
      return TYPE_PREFIX + EdlPresentationUtil.getName(typeDef, true);
    }
  ...
  public PsiNamedElement resolveElementByFQN(String s, Project project) {
    if (s.startsWith(FILE_PREFIX)) {
      ...
      return psiFile instanceof EdlFile ? psiFile : null;
    }

    if (s.startsWith(TYPE_PREFIX)) {
      String name = s.substring(TYPE_PREFIX.length());

      EdlIndexUtil.findTypeDef(project, Qn.fromDotSeparated(name), GlobalSearchScope.allScope(project));
    }
