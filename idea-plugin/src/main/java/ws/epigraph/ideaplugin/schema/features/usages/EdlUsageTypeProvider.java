...
import ws.epigraph.schema.parser.psi.EdlVarTagRef;
import org.jetbrains.annotations.Nullable;

/**
 * @author <a href="mailto:konstantin.sobolev@gmail.com">Konstantin Sobolev</a>
 */
public class EdlUsageTypeProvider implements UsageTypeProvider {
  @Nullable
  @Override
  public UsageType getUsageType(PsiElement element) {
    final PsiFile psiFile = element.getContainingFile();

    if (!(psiFile instanceof EdlFile)) return null;

    if (element instanceof EdlQnSegment) {
      if (PsiTreeUtil.getParentOfType(element, EdlImportStatement.class) != null) {
        return IMPORT_USAGE_TYPE;
      }
      return TYPE_REF_USAGE_TYPE; // be more precise: extends, list getElement type etc ?
    }

    if (PsiTreeUtil.getParentOfType(element, EdlVarTagRef.class) != null)
