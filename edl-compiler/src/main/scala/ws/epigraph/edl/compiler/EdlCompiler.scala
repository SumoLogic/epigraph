...
import ws.epigraph.edl.compiler.CPrettyPrinters._
...
class EdlCompiler(
  ...
  @throws[EdlCompilerException]("if compilation failed")
  ...
  @throws[EdlCompilerException]
  private def handleErrors(exitCode: Int): Unit = if (ctx.errors.nonEmpty) {
    renderErrors(ctx)
    throw new EdlCompilerException(exitCode.toString, ctx.errors, null)
  }
